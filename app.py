...
# Load models (ensure paths are correct and models exist)
try:
...
def predict():
    try:
        # Fetch all input values from the form
        input_values = [float(x) for x in request.form.values()]
        num_inputs = len(input_values)

        if num_inputs == 8:  # Diabetes prediction
        ...
        elif num_inputs == 10:  # Liver disease prediction
            Age = int(request.form['Age'])
        ...
        elif num_inputs == 13:  # Heart disease prediction
            age = int(request.form['age'])
        ...
        elif num_inputs == 18:  # Kidney disease prediction
            ...
            data = np.array([[age, bp, al, su, rbc, pc, pcc, ba, bgr, bu, sc, pot, wc, 
                              htn, dm, cad, pe, ane]])
            my_prediction = kidney_model.predict(data)

        else:
            return "Error: Unexpected number of input values."
